...
func main() {
	...
	readerOut := make(chan TransactionLine, 100000)
	go func(out chan TransactionLine) {
		...
		for i := 1; ; i++ {
			transactionLineStrings, err := reader.Read() //Read one line at a time

			if (err != nil) && (err == io.EOF) {
				break
			}
			amountFloat, _ := strconv.ParseFloat(transactionLineStrings[2], 64)
			out <- TransactionLine{PartnerName:transactionLineStrings[0], Currency:transactionLineStrings[1], Amount:amountFloat}
		}
	...
	//receive lines until channel is closed
	for i := 1; ; i++ {
		transactionLine, more := <-readerOut
		if more {

			aggregatedTransactions[transactionLine.PartnerName] += convertToHomeAmount(*homeCurrency, exchangeRates, transactionLine.Currency, transactionLine.Amount)
...
func convertToHomeAmount(homeCurrency string, exchangeRates map[Key]float64, transactionCurrency string, transactionAmount float64) float64 {
